...
local campBlips = campBlips or {} -- [campId] = blipHandle
local function normalizeCampId(id)
    local n = tonumber(id)
    return n or id
end

local function SetShortRange(blip, toggle)
    -- RDR2 native: SET_BLIP_AS_SHORT_RANGE (0xBE8BE4FE60E27B72)
    Citizen.InvokeNative(0xBE8BE4FE60E27B72, blip, toggle and true or false)
end
...
local function DrawCrosshair(isTarget)
    local dict, name = "blips", "blip_ambient_eyewitness"
    if not HasStreamedTextureDictLoaded(dict) then
        RequestStreamedTextureDict(dict); while not HasStreamedTextureDictLoaded(dict) do Wait(0) end
    end
    local r,g,b = 255,255,255; if isTarget then r,g,b = 0,255,0 end
...
local function loadModelHash(modelName)
    ...
    if not IsModelInCdimage(modelHash) or not IsModelValid(modelHash) then return nil end
    RequestModel(modelHash)
    local timeout = 0
    while not HasModelLoaded(modelHash) and timeout < 5000 do Wait(10); timeout = timeout + 10 end
...
local function safeCreateObject(modelHash, x, y, z)
    ...
    if not DoesEntityExist(obj) or obj == 0 then return nil end
...
-- NEW: Blip helpers
-- =========================
local function CreateCampBlip(campId, x, y, z, label)
    if not Config.Blips.enabled then return end
    local key = normalizeCampId(campId)
    if campBlips[key] and DoesBlipExist(campBlips[key]) then return end

    local blip
    if Config.Blips.useNativeAdd then
        blip = Citizen.InvokeNative(0x554D9D53F696D002, 1664425300, x + 0.0, y + 0.0, z + 0.0) -- BLIP_ADD_FOR_COORDS
    else
        blip = AddBlipForCoord(x + 0.0, y + 0.0, z + 0.0)
    end

    if Config.Blips.sprite ~= nil then
        SetBlipSprite(blip, Config.Blips.sprite, 1) -- works on RedM; 1 = filled
    end
    if type(Config.Blips.scale) == "number" then
        Citizen.InvokeNative(0xD38744167B2FA257, blip, Config.Blips.scale + 0.0) -- _SET_BLIP_SCALE
    end

    -- ❗ this is what was crashing; use the native hash:
    SetShortRange(blip, Config.Blips.shortRange)

    Citizen.InvokeNative(0x9CB1A1623062F402, blip, label or Config.Blips.defaultLabel or "My Camp") -- _SET_BLIP_NAME
    campBlips[key] = blip
end

local function RemoveCampBlip(campId)
    -- try both key forms defensively
    local keyNum = tonumber(campId)
    local keyStr = tostring(campId)
    local blip = (keyNum and campBlips[keyNum]) or campBlips[keyStr]
    if blip and DoesBlipExist(blip) then
        RemoveBlip(blip)
    end
    if keyNum then campBlips[keyNum] = nil end
    campBlips[keyStr] = nil
end
...
AddEventHandler('onResourceStop', function(res)
    ...
    for _, blip in pairs(campBlips) do
...
AddEventHandler('rs_camp:client:removeCamp', function(uniqueId)
    ...
    dynamicDoors[uniqueId] = nil
    -- do NOT remove blip here (non-owner). Owner gets a dedicated remove event.
end)

Citizen.CreateThread(function()
    TriggerServerEvent('rs_camp:server:requestCamps')
    -- NEW: also ask for my blips
    TriggerServerEvent('rs_camp:server:requestOwnCamps')
end)
...
-- NEW: bulk set my blips on join/reconnect
RegisterNetEvent('rs_camp:client:setCampBlips')
AddEventHandler('rs_camp:client:setCampBlips', function(rows)
    if not Config.Blips.enabled then return end
    for _, row in ipairs(rows or {}) do
        CreateCampBlip(row.id, row.x, row.y, row.z, Config.Blips.defaultLabel)
    end
end)

-- NEW: add/remove a single blip (owner-only)
RegisterNetEvent('rs_camp:client:addCampBlip')
AddEventHandler('rs_camp:client:addCampBlip', function(camp)
    if not camp or not camp.id then return end
    CreateCampBlip(camp.id, camp.x, camp.y, camp.z, Config.Blips.defaultLabel)
end)

RegisterNetEvent('rs_camp:client:removeCampBlip')
AddEventHandler('rs_camp:client:removeCampBlip', function(campId)
    if not campId then return end
    RemoveCampBlip(campId)
end)
...
CreateThread(function()
    while true do Wait(500); updateDoorPrompts() end
end)

CreateThread(function()
    while true do Wait(500); updateChestPrompts() end
end)
...
AddEventHandler('rs_camp:client:placePropCamp', function(itemName)
    ...
    CreateThread(function()
        while isPlacing do
            Wait(0)
            for _, keyCode in pairs(Config.Keys) do DisableControlAction(0, keyCode, true) end
            ...
            if IsDisabledControlJustPressed(0, Config.Keys.increaseSpeed) then
                posStep = math.min(posStep + 0.01, 5.0); rotStep = posStep * 10
                SendNUIMessage({ action = "showcamp", title = Config.ControlsPanel.title, controls = Config.ControlsPanel.controls, speed = Config.Text.SpeedLabel .. ": " .. string.format("%.2f", posStep) })
            end
            if IsDisabledControlJustPressed(0, Config.Keys.decreaseSpeed) then
                posStep = math.max(posStep - 0.01, 0.01); rotStep = posStep * 10
                SendNUIMessage({ action = "showcamp", title = Config.ControlsPanel.title, controls = Config.ControlsPanel.controls, speed = Config.Text.SpeedLabel .. ": " .. string.format("%.2f", posStep) })
            end
